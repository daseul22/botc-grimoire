...
- **관리자 비밀번호 초기화**: 관리자가 [/admin](../../app/admin/page.tsx)에서 대상 사용자의 비밀번호를
  임시 8자리(영숫자, 혼동문자 I/O/l/0/1 제외)로 재설정. 자체 change 로직(`changePassword`) 재사용이라
  **대상의 모든 세션을 폐기**(강제 재로그인)한다. 평문은 액션 응답에 **1회만** 반환·표시하고 저장하지 않으며,
  **본인 초기화는 막는다**(자기 세션까지 폐기돼 로그아웃되므로 — 본인은 `/account`). 정직히 relay 후 사용자가
  변경하도록 안내. [generateTempPassword](../../lib/auth.ts) · [resetPasswordAction](../../app/admin/actions.ts).
...
| `/admin` | 동적 | 사용자 역할 부여 · 비밀번호 초기화(임시 8자리)(관리자 전용). 마지막 관리자 강등 방지 |
