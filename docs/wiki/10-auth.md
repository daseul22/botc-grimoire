...
- **연결 확인(핵심)**: 변경 전, `countGamesByNickname`로 ① 새 닉네임으로 이미 기록된 게임
  (게스트/내역이 내 통계로 **흡수**됨)과 ② 현재 닉네임으로 집계된 기록(내 계정에서 **분리**됨)을
  보여주고, 둘 중 하나라도 있으면 **명시적 확인(confirm)** 을 받아야 적용한다. 사고성 오연결 방지.
  - 카운트는 **종료/진행 중**을 분리한다. 종료 게임 수는 통계 리더보드 집계 기준과 정확히 일치하고
    (둘 다 `status='finished'`만 셈), 진행 중 게임은 "종료 시 연결됨"으로 따로 표기한다.
