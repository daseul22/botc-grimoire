...
- Townsfolk → **마을주민**(이전 '마을주민'·'주민' 폐기), Outsider → **외지인**(이전 '아웃사이더'·'외부인' 폐기)
...
### 2차 정리 — 코드·동작 데이터의 폐기어

`data/characters.json`은 위와 같이 통일됐지만, **코드와 동작 데이터**(`data/behaviors.json`·컴포넌트
UI 문구·추천 근거)에는 폐기어가 남아 있었다. 같은 권고 표준으로 정리했다.

| 폐기어 | 정규어 | 비고 |
|---|---|---|
| 데몬 | **악마** | 받침이 사라져 조사도 함께 보정(`데몬이`→`악마가`, `데몬으로`→`악마로` 등) |
| 마을사람 | **마을주민** | |
| 레크루스 / 스파이 | **은둔자 / 첩자** | 코드가 이미 그렇게 부르는데 문구만 음차로 남아 있었다 |

- `data/characters.json`의 `악마이며`·`악마이거나`는 **서술격 조사**라 올바른 형태다 — 정규화에서 제외한다.
- [번역 일관성 조사 보고서](../번역-일관성-조사-보고서.md)는 용어표에 폐기어를 인용하고 있어(역사적 기록)
  치환 대상에서 뺐다.
- `npm run verify:behaviors`는 **원본 쪽에만** 같은 정규화를 적용해 비교한다 — 이 용어 변경은
  통과시키되, 현재 값에 폐기어가 남아 있으면 불일치로 드러나 누락 검출기 역할을 한다.
