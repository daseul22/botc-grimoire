...
[← 인증·인가](10-auth.md) · [홈](README.md) · 다음: [커스텀 직업 →](12-custom-characters.md)
...
- **반응형 레이아웃**: 데스크탑(`lg`+)은 보드(좌, `flex-1`·정사각 `self-start`) + 자유 메모(우측 사이드바
  `w-80`, 보드 높이에 맞춰 크게)로 2열. 모바일은 세로 스택(보드 → 메모).
- **스크립트 직업 목록**: 상단 "직업 목록" 버튼으로 여는 **우측 오버레이 드로어**(`scriptOpen`). 메모 공간을
...
- **마도서 열람 직업**(첩자 — `showcase.mode: "grimoire"`): **📲 보여주기** → 그 좌석 폰에 **전체 그리모어**를 push → **확인했습니다**.
  **이야기꾼 캔버스와 동일한 원형 좌석 배치**(payload에 좌표 x/y — 플레이어 보드와 같은 `posPct=(0.1+v·0.8)·100%` 인셋)로
  각 토큰에 실제 직업 아이콘·직업명·닉네임·진영(악=붉은 테두리)·생사(사망 글리프)를 그리고, **상태 토큰(마커)은 ST 캔버스와
  동일하게 `MarkerToken`으로**(보호·독·사망예정·집착 등 실제 리마인더 토큰 아이콘 — roleParam 마커는 대상 직업 토큰까지). 기록 불필요(현재 보드 스냅샷 그대로).
  payload(`kind:"grimoire"`)에 전 좌석 정체가 담기지만 **받는 좌석에만** 내려가 자기완결(seat 게이팅). 기록 목록엔 요약만("마도서 확인").
  전체 보드라 delivered 모달은 넓게(`max-w-3xl`, 보드 `max-w-[min(100%,72vh)]` — 15명까지). 좌표가 없는 구 payload는 `circlePositions` **원형 폴백**으로 배치(토큰 겹침 방지).
