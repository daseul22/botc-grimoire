...
// 전챗(전체)은 낮에 가능하되 '지목 시간(지목 받기 열림~투표)'엔 차단. 밤은 전부 차단. 이야기꾼(운영자)은 예외(호출측에서 제외).
 ...
 * mySeat=발신자 좌석(관전/미배정이면 null → 귓말 불가).
 * nominationsActive=지목 단계 활성 — ST가 '지목 받기'를 열었거나(nominationsOpen) 활성 지목/투표가 진행 중.
 */
export function chatGate(
  phase: string,
  openRunning: boolean,
  nominationsActive: boolean,
  mySeat: number | null,
  seatCount: number,
): ChatGate {
  if (phase === "night")
    return { allChat: false, whisper: false, neighborSeats: [], reason: "밤에는 대화할 수 없습니다." };
  if (nominationsActive)
    return { allChat: false, whisper: false, neighborSeats: [], reason: "지목 시간에는 대화할 수 없습니다." };
